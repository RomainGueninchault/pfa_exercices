...
void print_test_result(const struct test *test, int status)
{
    ...
    fprintf(stderr, "** "COLOR(TEST, "Test %s")" **\t%s (%d/%d)\n",
            test->name,
            failed ? COLOR(FAILED, "FAILED") : COLOR(PASSED, "PASSED"), assert_passed, test->len);
    fflush(stderr);
}
...
{
    (void) test;
    fprintf(stdout, "; %2d ; %2d", !(status & 1), status >> 1);
    fflush(stdout);
}

void print_named_result(const struct test *test, int status)
{
    (void) test;
    if (status & 1)
        fprintf(stderr, "Test %s fails ! Assert passed %d/%d\n",
                test->name, status >> 1, test->len);
    fprintf(stdout, "; %s ; %2d", test->name, status >> 1);
    fflush(stdout);
}
...
#define PAGINATE_BY 4

static void print_compact_summary_helper(const char *title, int failed)
{
    int count = 0;
    fputs(title, stderr);
    fputs("\n  ", stderr);
    for (int i = 0; i < _runned_count; i ++)
        if (!HAS_FAILED(_runned_tests[i].status) == !failed) {
            fprintf(stderr, " %s(%d/%d)" ,
                    _runned_tests[i].test->name, _runned_tests[i].status >> 1, _runned_tests[i].test->len);
...
void print_compact_summary()
{
    ...
    if (_failed_count != _runned_count)
        print_compact_summary_helper("Test passed:", 0);
    if (_failed_count != 0)
        print_compact_summary_helper("Test failed:", 1);
}

struct printer print_raw = { .result = print_raw_test_result };
struct printer print_named = { .result = print_named_result };
...
static void segfault(int signal, siginfo_t *siginfo, void *context)
{
    ...
    segfault_msg(signal);
    // print_trace();
    EXIT(_tests_count ? _test_passed : _assert_passed, EXIT_FAILURE);
}
...
static int execute_test(const struct test *test)
{
    ...
    switch(pid = fork()) {
        ...
        case 0: // Child
            ...
            EXIT(_tests_count ? _test_passed : _assert_passed, status != 0);
        default: // Parent
    ...
    while(tests[++i].name) {
        ...
        do {
            if (!strcmp(test_name, name))
                return &tests[i];
            test_name = strchr(test_name, ':');
        } while (test_name && test_name[0] == ':' && test_name ++); // ugly way to skip ':'
    }
    return NULL;
}

int set_tests_count(int test_count)
{
    _tests_count = test_count;
    return 0; // return is only for chaining
}
int test_passed()
{
    return ++_test_passed;
...
// This is a debug function, it can be used but the prtotype is not in grader2.h
...
PROVIDED_FUNCTION(int, alt_main, int argc, char** argv)
{
    (void) argc;
    (void) argv;
...
int grader(const struct test *exam, int argc, char **argv)
{
    ...
    while ((ch = getopt(argc, argv, "mcrbLh?l")) != -1) {
        switch (ch) {
            case 'c':
            case 'r':
                printer = &print_raw;
...
{
    return (grader_full_compartor)simple_comparator; // FIXME how to mute this very particular warning
}

/**
 * returns the first mismatching index, -1 if they match
 */
int array_compare(grader_full_compartor comparator,
        const void *a1, const void *a2, size_t n, size_t item_size, double prec)
// FIXME comparator should now be compatible with memcmp
{
    const char *c1 = a1;
    const char *c2 = a2;
    for (size_t i = 0; i < n; i ++)
        if (comparator(c1 + i*item_size, c2 + i*item_size, item_size, prec) == 0) // they diverged
            return i;
    return -1;
}

/**
 * returns number of missings items, 0 if they match
 */
int array_compare_unordered_missings_fp(grader_full_compartor comparator,
    const void *a1, const void *a2, size_t items, size_t item_size, double prec, void *m1[], void *m2[])
// FIXME comparator should now be compatible with memcmp
{
#       define AT(__base, __item) \
            ((__base) + (__item)*item_size)
    size_t missings = items;
    const void *v2[items];
    ...
    for (size_t i = 0; i < items; i ++) {
        for (size_t j = 0; j < missings; j ++) {
            if (comparator(AT(a1, i), v2[j], item_size, prec) != 0) { // there is a match
                if (--missings>0) // Repack
                    v2[j] = v2[missings];
                goto next;
            }
        }
        // mismatch
        if (m1) *m1++ = (void*)AT(a1, i); // save it
next:
        ;
    }
    if (m2 && missings) memcpy(m2, v2, missings*sizeof(void*));
    return missings;
#       undef AT
}
